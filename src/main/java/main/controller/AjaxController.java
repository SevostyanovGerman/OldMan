...
public class AjaxController {
	...
	public List<String> getUsersByNameLike(@PathVariable String name) {
			return userService.getUsersByNameLike(name).stream().map(User::getName).filter(newName -> !name.equals(userService.getCurrentUser().getName())).collect(Collectors.toList());
	}
